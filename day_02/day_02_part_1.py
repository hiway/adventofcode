...
def wrapping_paper_estimate(length, width, height):
...
def feed_wrapping_paper_estimates():
    ...
    for dimensions in boxes:
        # Remove spaces, lowercase (just in case) and split on 'x'
        dimensions = dimensions.strip().lower().split('x')
        # Convert to integers
        dims = [int(y) for y in dimensions]
        # Add to total
        total += wrapping_paper_estimate(*dims)
    print(total)


if __name__ == '__main__':
    feed_wrapping_paper_estimates()
